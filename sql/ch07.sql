...
DELETE FROM 부서
WHERE NOT EXISTS(SELECT 1 FROM 사원 WHERE 부서.부서번호 = 사원.부서번호) 
 -- select 1은 존재 여부만 확인하는 역할을 한다. not exists에서 행이 존재하는지만 체크한다.
...
-- 서브쿼리를 사용해서
-- CTE을 사용해서
SELECT AVG(마일리지) INTO @avgMileage
FROM 고객
WHERE 담당자직위 = '대표 이사';
SELECT @avgMileage;
-- 대표이사 평균 마일리지를 변수에 넣음
UPDATE 고객
SET 마일리지 = @avgMileage
WHERE 고객번호 = 'ZZZAA';
-- ------------3번 강사님풀이
