...
SELECT * FROM 제품
WHERE NOT EXISTS (
	SELECT 제품번호 FROM 주문세부
	WHERE 제품번호 = 제품.제품번호
);
-- 3. 담당자명, 고객회사명, 주문건수, 최초주문일과 최종주문일을 보이시오.
SELECT 
	담당자명, 고객회사명, 주문요약.주문건수, 주문요약.최초주문일, 주문요약.최종주문일
FROM 고객
	,(SELECT
		count(*) AS 주문건수, min(주문일) AS 최초주문일, max(주문일) AS 최종주문일, 고객번호
	FROM 주문
	GROUP BY 고객번호
	) AS 주문요약
...
SELECT 제품명, 단가, 
	(SELECT sum(주문수량) FROM 주문세부 WHERE 제품번호=제품.제품번호) AS 주문수량합
FROM 제품
WHERE 단가 >= ALL(SELECT 단가 FROM 제품);
