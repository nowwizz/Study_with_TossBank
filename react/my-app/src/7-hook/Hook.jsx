...
//App7.js import * as E from "7-hook"
...
//함수형 컴포넌트의 생명주기 함수를 지원한다.
//1. 마운트: useEffect(() => {}, []);
//2. 언마운트: useEffect(() => {return () => {} });
//3. 업데이트(props, state가 데이트될 때): useEffect(() => {}, [state1, state2]);
...
//3. useMemo 훅
//      useMemo는 특정 연산의 결과를 메모리제이션(저장하여)
//      불필요한 재계산을 방지하는 데 사용된다.
//      컴포넌트가 렌더링될 때마다 매번 계산하는 대신
...
export function Counter4() {
  ...
  //number 상태변수가 변경되지 않는 한, double 값은 재연산되지 않음
...
export function Counter6() {
  const [count, setCount] = useState(0);
  const clickCountRef = useRef(0); //useRef훅으로 클릭 횟수 관리
  //clickCountRef는 컴포넌트가 리렌더링될 때 값이 초기화되지 않는다.
