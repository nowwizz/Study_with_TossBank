...
export function NumberList() {
  ...
  return (
    ...
    키를 안쓰면 이런 에러가 뜬다. 키 속성이 없다는 오류. */}
        {/* 배열의 인덱스를 사용하는 것은 오류는 사라지지만 그리 좋은 방법이 아니다.
        배열 인덱스를 키로 사용하면 배열의 데이터의 순서가 바뀌거나 추가/삭제가 되면, 리액트 모든 항목을 다시 렌더링해야 됨.
        배열 인덱스를 키로 사용하면 입력의 포커스나 애니메이션효과 등의 상태를 유지할 수 없다.*/}
        {nums.map((number, index) => (
          <li key={index}>{number}</li> //리턴 생략했으니 소괄호로
        ))}
      </ul>
    </>
  );
